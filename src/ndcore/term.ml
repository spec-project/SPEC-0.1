...
type state = int
(* AT: modified for testing experimental features *)
let bind_stack = ref (Stack.create ())  
let bind_len = ref 0
...
let marshal_symbols () = Marshal.to_string (!symbols) [] 
let unmarshal_symbols s = 
   let sym : term NS.t = Marshal.from_string s 0 in 
   symbols := sym

let marshal_bind_stack () = Marshal.to_string (!bind_stack) []
let unmarshal_bind_stack s = 
  let bs : (ptr * in_ptr) Stack.t = Marshal.from_string s 0 in
    bind_stack := bs

let marshal_bind_len () = Marshal.to_string (!bind_len) []
let unmarshal_bind_len s =
  let i : int = Marshal.from_string s 0 in
    bind_len := i 
